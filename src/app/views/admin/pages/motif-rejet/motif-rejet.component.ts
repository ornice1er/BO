...
import { MotifRejetService } from '../../../../core/services/motif-rejet.service';
...
export class MotifRejetComponent implements OnInit {

  selected_data: any;
  user: any;
  add_data: any = { allow_complement: true, is_final: false, is_active: true };
  data: any[] = [];
  etapes: any[] = [];
  prestations: any[] = [];
  permissions: any[] = [];
  ...
  constructor(
    private service: MotifRejetService,
  ...
  store(value: any) {
    ...
    const payload = {
      ...value,
      allow_complement: this.add_data.allow_complement,
      is_final: this.add_data.is_final,
      is_active: this.add_data.is_active,
    };
  ...
  onLibelleChange(libelle: string) {
    this.add_data.code = libelle
      .toUpperCase()
  ...
  onSearchChange() {
    const localResults = this.data.filter((d: any) =>
      d.libelle?.includes(this.search_text) || d.code?.includes(this.search_text)
    );
