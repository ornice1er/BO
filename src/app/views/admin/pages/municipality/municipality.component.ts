...
export class MunicipalityComponent implements OnInit {
 ...
 this.modalService.open(content, { size: 'lg' }); }
  ...
  store(value: any) {
    this.loading = true;
    this.municipalityService.store(value).subscribe(
  ...
  update(value: any) {
    this.loading = true;
    this.municipalityService.update(value, this.selected_data.id).subscribe(
      () => { this.loading = false; this.modalService.dismissAll(); this.all(); },
      () => { this.loading = false; }
    );
  }
  ...
  async delete() {
    ...
    if (result.isConfirmed) {
      this.loading = true;
      this.municipalityService.delete(this.selected_data.id).subscribe(
  ...
  setStatus(value: any) {
    ...
    this.loading = true;
    this.municipalityService.setStatus(this.selected_data.id, value).subscribe(
      (res: any) => { this.toastrService.success(res.message); this.loading = false; this.all(); },
      (err: any) => { this.loading = false; AppSweetAlert.simpleAlert('error', 'Municipalités', err.error.message); }
    );
  }
  ...
  onSearchChange() {
    ...
    if (this.search_text.length > 2 && local.length === 0) {
      this.municipalityService.search({ search: this.search_text }).subscribe({
