import {
  ...
  reduce_default,
  uniqBy_default
} from "./chunk-ILT6OF2W.js";
import {
  isEmpty_default
} from "./chunk-OBVX5ZNT.js";
import {
  __async,
  __commonJS,
  __esm,
...
`;
      ...
      const connection = {
        ...
        trace: (_value, _tracer, sendNotificationOrTraceOptions) => __async(null, null, function* () {
          ...
          if (_sendNotification && !isClosed() && !isDisposed()) {
            yield connection.sendNotification(SetTraceNotification.type, {
  ...
  tracer("Transform Patterns", () => {
    ...
    allTransformedPatterns = onlyRelevantTypes.map((currType) => {
      ...
      if (currPattern instanceof RegExp) {
        ...
        !["d", "D", "s", "S", "t", "r", "n", "t", "0", "c", "b", "B", "f", "v", "w", "W"].includes(regExpSource[1])) {
...
function validateLookahead(options) {
  ...
  return lookaheadValidationErrorMessages.map((errorMessage) => Object.assign({
...
var LooksAhead = class {
  initLooksAhead(config) {
    ...
    this.lookaheadStrategy = Object.hasOwn(config, "lookaheadStrategy") ? config.lookaheadStrategy : new LLkLookaheadStrategy({
    ...
    rules.forEach((currRule) => {
      this.TRACE_INIT(`${currRule.name} Rule Lookahead`, () => {
        const {
          ...
          option: option2,
          repetitionMandatory: repetitionMandatory2,
...
applyMixins(Parser, [Recoverable, LooksAhead, TreeBuilder, LexerAdapter, RecognizerEngine, RecognizerApi, ErrorHandler, GastRecorder, PerformanceTracer]);
...
var AbstractLangiumParser = class {
  constructor(services, incomplete) {
    ...
    if (services.shared.profilers.LangiumProfiler?.isActive("parsing")) {
      this.wrapper = new ProfilerWrapper(tokens, __spreadProps(__spreadValues({}, services.parser.ParserConfig), {
      ...
      this.wrapper = new ChevrotainWrapper(tokens, __spreadProps(__spreadValues({}, services.parser.ParserConfig), {
        skipValidations: production,
        errorMessageProvider: services.parser.ParserErrorMessageProvider
      }), incomplete);
    }
...
var ChevrotainWrapper = class extends EmbeddedActionsParser {
  constructor(tokens, config, incomplete) {
    ...
    super(tokens, __spreadValues(__spreadProps(__spreadValues({}, defaultConfig), {
...
function buildRules(parserContext, grammar) {
  ...
  for (const rule of parserRules) {
    const ctx = __spreadProps(__spreadValues({}, parserContext), {
      consume: 1,
      optional: 1,
      subrule: 1,
      many: 1,
...
function buildInfixRule(ctx, rule) {
  ...
  const part2Assignment = __spreadProps(__spreadValues({}, part1Assignment), {
    $container: innerGroup
  });
...
(() => {
  ...
  !function(t2) {
    t2.joinPath = function(t3, ...e2) {
      ...
      return n2 && i2[0] === _ && !t3.authority && (i2 = i2.substring(1)), t3.with({
        path: i2
      });
...
var DefaultLangiumDocumentFactory = class {
  ...
  fromUri(_0) {
    return __async(this, arguments, function* (uri, cancellationToken = cancellation_exports.CancellationToken.None) {
      const content = yield this.fileSystemProvider.readFile(uri);
  ...
  createAsync(uri, content, cancelToken) {
    return __async(this, null, function* () {
      if (typeof content === "string") {
        const parseResult = yield this.parseAsync(uri, content, cancelToken);
        return this.createLangiumDocument(parseResult, uri, void 0, content);
      } else {
        const parseResult = yield this.parseAsync(uri, content.getText(), cancelToken);
        return this.createLangiumDocument(parseResult, uri, content);
      }
    });
  }
  ...
  update(document, cancellationToken) {
    return __async(this, null, function* () {
      ...
      const text = textDocument ? textDocument.getText() : yield this.fileSystemProvider.readFile(document.uri);
      if (textDocument) {
      ...
      if (oldText !== text) {
        document.parseResult = yield this.parseAsync(document.uri, text, cancellationToken);
...
var DefaultLangiumDocuments = class {
  ...
  getOrCreateDocument(uri, cancellationToken) {
    return __async(this, null, function* () {
      let document = this.getDocument(uri);
      if (document) {
        return document;
      }
      document = yield this.langiumDocumentFactory.fromUri(uri, cancellationToken);
...
var DefaultLinker = class {
  ...
  link(_0) {
    return __async(this, arguments, function* (document, cancelToken = cancellation_exports.CancellationToken.None) {
      if (this.profiler?.isActive("linking")) {
        const task = this.profiler.createTask("linking", this.languageId);
        task.start();
        try {
          for (const node of streamAst(document.parseResult.value)) {
            yield interruptAndCheck(cancelToken);
    ...
    const reference = {
      ...
      get error() {
        ...
        if (refs.length > 0) {
          ...
          return this._linkingError = linker.createLinkingError({
            reference,
            container: node,
            property
          });
...
var DefaultScopeComputation = class {
   ...
   */
  collectExportedSymbolsForNode(_0, _1) {
    return __async(this, arguments, function* (parentNode, document, children = streamContents, cancelToken = cancellation_exports.CancellationToken.None) {
      const exports = [];
...
var ValidationRegistry = class {
  ...
  wrapValidationException(check, thisObj) {
    return (node, accept, cancelToken) => __async(this, null, function* () {
      yield this.handleException(() => check.call(thisObj, node, accept, cancelToken), "An error occurred during validation", accept, node);
    });
  }
  handleException(functionality, messageContext, accept, node) {
    return __async(this, null, function* () {
      try {
        yield functionality();
      } catch (err) {
  ...
  wrapPreparationException(check, messageContext, thisObj) {
    return (rootNode, accept, categories, cancelToken) => __async(this, null, function* () {
      yield this.handleException(() => check.call(thisObj, rootNode, accept, categories, cancelToken), messageContext, accept, rootNode);
    });
  }
  get checksBefore() {
...
var DefaultDocumentValidator = class {
  ...
  validateDocument(_0) {
    return __async(this, arguments, function* (document, options = {}, cancelToken = cancellation_exports.CancellationToken.None) {
      ...
      try {
        diagnostics.push(...yield this.validateAst(parseResult.value, options, cancelToken));
      } catch (err) {
        if (isOperationCancelled(err)) {
          throw err;
        }
  ...
  validateAst(_0, _1) {
    return __async(this, arguments, function* (rootNode, options, cancelToken = cancellation_exports.CancellationToken.None) {
      const validationItems = [];
      const acceptor = (severity, message, info) => {
        validationItems.push(this.toDiagnostic(severity, message, info));
      };
      yield this.validateAstBefore(rootNode, options, acceptor, cancelToken);
      yield this.validateAstNodes(rootNode, options, acceptor, cancelToken);
      yield this.validateAstAfter(rootNode, options, acceptor, cancelToken);
      return validationItems;
    });
  }
  validateAstBefore(_0, _1, _2) {
    return __async(this, arguments, function* (rootNode, options, acceptor, cancelToken = cancellation_exports.CancellationToken.None) {
      const checksBefore = this.validationRegistry.checksBefore;
      for (const checkBefore of checksBefore) {
        yield interruptAndCheck(cancelToken);
        yield checkBefore(rootNode, acceptor, options.categories ?? [], cancelToken);
      }
    });
  }
  validateAstNodes(_0, _1, _2) {
    return __async(this, arguments, function* (rootNode, options, acceptor, cancelToken = cancellation_exports.CancellationToken.None) {
  ...
  validateAstAfter(_0, _1, _2) {
    return __async(this, arguments, function* (rootNode, options, acceptor, cancelToken = cancellation_exports.CancellationToken.None) {
      const checksAfter = this.validationRegistry.checksAfter;
      for (const checkAfter of checksAfter) {
        yield interruptAndCheck(cancelToken);
        yield checkAfter(rootNode, acceptor, options.categories ?? [], cancelToken);
      }
    });
  }
...
var DefaultConfigurationProvider = class {
    ...
    return __async(this, null, function* () {
      if (this.workspaceConfig) {
        ...
        if (params.fetchConfiguration) {
          ...
          const configs = yield params.fetchConfiguration(configToUpdate);
  ...
  */
  getConfiguration(language, configuration) {
    return __async(this, null, function* () {
      yield this.ready;
...
(function(Disposable2) {
  function create(callback) {
    return {
      dispose: () => __async(null, null, function* () {
...
var DefaultDocumentBuilder = class {
  ...
  build(_0) {
    return __async(this, arguments, function* (documents, options = {}, cancelToken = cancellation_exports.CancellationToken.None) {
      ...
      this.currentState = DocumentState.Changed;
      yield this.emitUpdate(documents.map((e) => e.uri), []);
      yield this.buildDocuments(documents, options, cancelToken);
    });
  }
  update(_0, _1) {
    return __async(this, arguments, function* (changed, deleted, cancelToken = cancellation_exports.CancellationToken.None) {
      this.currentState = DocumentState.Changed;
      const deletedUris = [];
      ...
      const changedUris = (yield Promise.all(changed.map((uri) => this.findChangedUris(uri)))).flat();
      for (const changedUri of changedUris) {
        ...
        if (changedDocument === void 0) {
          changedDocument = this.langiumDocumentFactory.fromModel({
            $type: "INVALID"
          }, changedUri);
      ...
      yield this.emitUpdate(changedUris, deletedUris);
      yield interruptAndCheck(cancelToken);
      ...
      yield this.buildDocuments(rebuildDocuments, this.updateBuildOptions, cancelToken);
    });
  }
  ...
  findChangedUris(changed) {
    return __async(this, null, function* () {
      ...
      try {
        const stat = yield this.fileSystemProvider.stat(changed);
        if (stat.isDirectory) {
          const uris = yield this.workspaceManager().searchFolder(changed);
          return uris;
  ...
  emitUpdate(changed, deleted) {
    return __async(this, null, function* () {
      yield Promise.all(this.updateListeners.map((listener) => listener(changed, deleted)));
    });
  }
  /**
  ...
  buildDocuments(documents, options, cancelToken) {
    return __async(this, null, function* () {
      ...
      yield this.runCancelable(documents, DocumentState.Parsed, cancelToken, (doc) => this.langiumDocumentFactory.update(doc, cancelToken));
      yield this.runCancelable(documents, DocumentState.IndexedContent, cancelToken, (doc) => this.indexManager.updateContent(doc, cancelToken));
      yield this.runCancelable(documents, DocumentState.ComputedScopes, cancelToken, (doc) => __async(this, null, function* () {
        const scopeComputation = this.serviceRegistry.getServices(doc.uri).references.ScopeComputation;
        doc.localSymbols = yield scopeComputation.collectLocalSymbols(doc, cancelToken);
      }));
      const toBeLinked = documents.filter((doc) => this.shouldLink(doc));
      yield this.runCancelable(toBeLinked, DocumentState.Linked, cancelToken, (doc) => {
        const linker = this.serviceRegistry.getServices(doc.uri).references.Linker;
        return linker.link(doc, cancelToken);
      });
      yield this.runCancelable(toBeLinked, DocumentState.IndexedReferences, cancelToken, (doc) => this.indexManager.updateReferences(doc, cancelToken));
      ...
      yield this.runCancelable(toBeValidated, DocumentState.Validated, cancelToken, (doc) => __async(this, null, function* () {
        yield this.validate(doc, cancelToken);
        this.markAsCompleted(doc);
      }));
    });
  }
    ...
    return __async(this, null, function* () {
      for (const document of documents) {
        if (document.state < targetState) {
          ...
          yield this.notifyDocumentPhase(document, targetState, cancelToken);
        }
      }
      const targetStateDocs = documents.filter((doc) => doc.state === targetState);
      yield this.notifyBuildPhase(targetStateDocs, targetState, cancelToken);
  ...
  notifyDocumentPhase(document, state, cancelToken) {
    return __async(this, null, function* () {
      ...
      for (const listener of listenersCopy) {
        try {
          yield interruptAndCheck(cancelToken);
          yield listener(document, cancelToken);
        } catch (err) {
          if (!isOperationCancelled(err)) {
            throw err;
          }
   ...
   */
  validate(document, cancelToken) {
    return __async(this, null, function* () {
      const validator = this.serviceRegistry.getServices(document.uri).validation.DocumentValidator;
      const options = this.getBuildOptions(document);
      const validationOptions = typeof options.validation === "object" ? __spreadValues({}, options.validation) : {};
      validationOptions.categories = this.findMissingValidationCategories(document, options);
      const diagnostics = yield validator.validateDocument(document, validationOptions, cancelToken);
...
var DefaultIndexManager = class {
  ...
  updateContent(_0) {
    return __async(this, arguments, function* (document, cancelToken = cancellation_exports.CancellationToken.None) {
      ...
      const exports = yield services.references.ScopeComputation.collectExportedSymbols(document, cancelToken);
  ...
  updateReferences(_0) {
    return __async(this, arguments, function* (document, cancelToken = cancellation_exports.CancellationToken.None) {
      const services = this.serviceRegistry.getServices(document.uri);
      const indexData = yield services.workspace.ReferenceDescriptionProvider.createDescriptions(document, cancelToken);
...
var DefaultWorkspaceManager = class {
  ...
  initializeWorkspace(_0) {
    return __async(this, arguments, function* (folders, cancelToken = cancellation_exports.CancellationToken.None) {
      const documents = yield this.performStartup(folders);
      yield interruptAndCheck(cancelToken);
      yield this.documentBuilder.build(documents, this.initialBuildOptions, cancelToken);
    });
  }
  /**
  ...
  performStartup(folders) {
    return __async(this, null, function* () {
      ...
      const uris = [];
      yield Promise.all(folders.map((wf) => this.getRootFolder(wf)).map((entry) => __async(this, null, function* () {
        return this.traverseFolder(entry, uris);
      })));
      const uniqueUris = stream(uris).distinct((uri) => uri.toString()).filter((uri) => !this.langiumDocuments.hasDocument(uri));
      yield this.loadWorkspaceDocuments(uniqueUris, collector);
      this._ready.resolve();
      return documents;
    });
  }
  loadWorkspaceDocuments(uris, collector) {
    return __async(this, null, function* () {
      yield Promise.all(uris.map((uri) => __async(this, null, function* () {
        const document = yield this.langiumDocuments.getOrCreateDocument(uri);
        collector(document);
      })));
    });
  }
   ...
   */
  traverseFolder(folderPath, uris) {
    return __async(this, null, function* () {
      try {
        const content = yield this.fileSystemProvider.readDirectory(folderPath);
        yield Promise.all(content.map((entry) => __async(this, null, function* () {
          if (this.shouldIncludeEntry(entry)) {
            if (entry.isDirectory) {
              yield this.traverseFolder(entry.uri, uris);
  ...
  searchFolder(uri) {
    return __async(this, null, function* () {
      const uris = [];
      yield this.traverseFolder(uri, uris);
      return uris;
    });
  }
  /**
...
var AbstractThreadedAsyncParser = class {
  ...
  parse(text, cancelToken) {
    return __async(this, null, function* () {
      const worker = yield this.acquireParserWorker(cancelToken);
  ...
  acquireParserWorker(cancelToken) {
    return __async(this, null, function* () {
...
var DefaultWorkspaceLock = class {
  ...
  performNextOperation() {
    return __async(this, null, function* () {
      ...
      this.done = false;
      yield Promise.all(entries.map((_0) => __async(this, [_0], function* ({
        action,
        deferred,
        cancellationToken
      }) {
        try {
          const result = yield Promise.resolve().then(() => action(cancellationToken));
...
var DefaultHydrator = class {
  ...
  dehydrate(result) {
    return {
      ...
      parserErrors: result.parserErrors.map((e) => __spreadProps(__spreadValues({}, e), {
        message: e.message
      })),
...
var IndentationAwareTokenBuilder = class extends DefaultTokenBuilder {
  constructor(options = indentationBuilderDefaultOptions) {
    ...
    this.options = __spreadValues(__spreadValues({}, indentationBuilderDefaultOptions), options);
...
var MermaidTerminals = __spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues({}, ArchitectureGrammar.Terminals), GitGraphGrammar.Terminals), InfoGrammar.Terminals), PacketGrammar.Terminals), PieGrammar.Terminals), RadarGrammar.Terminals), TreeViewGrammar.Terminals), TreemapGrammar.Terminals), WardleyGrammar.Terminals);
var Accelerator = {
  $type: "Accelerator",
  name: "name",
  x: "x",
...
//# sourceMappingURL=chunk-TJULHAOM.js.map
