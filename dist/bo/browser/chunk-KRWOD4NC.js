import {
  populateCommonDb
} from "./chunk-7YZQQ5OT.js";
import {
  selectSvgElement
} from "./chunk-H2D4YPZ3.js";
import {
  parse
} from "./chunk-U4B7RYOM.js";
...
var ArchitectureDB = class {
  ...
  getDataStructures() {
    if (this.dataStructures === void 0) {
      ...
      const notVisited = Object.keys(adjList).reduce((prev, id) => id === firstId ? prev : __spreadProps(__spreadValues({}, prev), {
        [id]: 1
      }), {});
...
var populateDb = /* @__PURE__ */ __name((ast, db) => {
  ...
  ast.services.map((service) => db.addService(__spreadProps(__spreadValues({}, service), {
    type: "service"
  })));
  ast.junctions.map((service) => db.addJunction(__spreadProps(__spreadValues({}, service), {
    type: "junction"
  })));
...
var parser = {
  ...
  parse: /* @__PURE__ */ __name((input) => __async(null, null, function* () {
    const ast = yield parse("architecture", input);
    log.debug(ast);
...
var drawEdges = /* @__PURE__ */ __name(function(edgesEl, cy, db, diagramId) {
  return __async(this, null, function* () {
    ...
    yield Promise.all(cy.edges().map((edge) => __async(null, null, function* () {
      const {
        source,
        sourceDir,
        sourceArrow,
      ...
      if (edge[0]._private.rscratch) {
        ...
        g.insert("path").attr("d", `M ${startX},${startY} L ${midX},${midY} L${endX},${endY} `).attr("class", "edge").attr("id", `${diagramId}-${getEdgeId(source, target, {
          prefix: "L"
        })}`);
        if (sourceArrow) {
        ...
        if (label) {
          ...
          yield createText(textElem, label, {
            useHtmlLabels: false,
            width,
...
var drawGroups = /* @__PURE__ */ __name(function(groupsEl, cy, db, diagramId) {
  return __async(this, null, function* () {
    const padding = db.getConfigField("padding");
    ...
    const iconSize = db.getConfigField("iconSize");
    const halfIconSize = iconSize / 2;
    yield Promise.all(cy.nodes().map((node) => __async(null, null, function* () {
      ...
      if (data.type === "group") {
        ...
        if (data.icon) {
          ...
          bkgElem.html(`<g>${yield getIconSVG(data.icon, {
        ...
        if (data.label) {
          ...
          yield createText(textElem, data.label, {
            useHtmlLabels: false,
            width: w,
            classes: "architecture-service-label"
          }, getConfig2());
...
var drawServices = /* @__PURE__ */ __name(function(db, elem, services, diagramId) {
  return __async(this, null, function* () {
    ...
    for (const service of services) {
      ...
      if (service.title) {
        ...
        yield createText(textElem, service.title, {
          useHtmlLabels: false,
          width: iconSize * 1.5,
          classes: "architecture-service-label"
        }, config);
      ...
      if (service.icon) {
        bkgElem.html(`<g>${yield getIconSVG(service.icon, {
          height: iconSize,
          width: iconSize,
          fallbackPrefix: architectureIcons.prefix
        })}</g>`);
      } else if (service.iconText) {
        bkgElem.html(`<g>${yield getIconSVG("blank", {
          height: iconSize,
          width: iconSize,
          fallbackPrefix: architectureIcons.prefix
        })}</g>`);
...
function layoutArchitecture(services, junctions, groups, edges, db, {
  spatialMaps,
  groupAlignments
}) {
...
var draw = /* @__PURE__ */ __name((text, id, _version, diagObj) => __async(null, null, function* () {
  const db = diagObj.db;
  db.setDiagramId(id);
  ...
  yield drawServices(db, servicesElem, services, id);
  drawJunctions(db, servicesElem, junctions, id);
  const cy = yield layoutArchitecture(services, junctions, groups, edges, db, ds);
  yield drawEdges(edgesElem, cy, db, id);
  yield drawGroups(groupElem, cy, db, id);
  positionNodes(db, cy);
...
//# sourceMappingURL=chunk-KRWOD4NC.js.map
