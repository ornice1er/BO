...
var recursiveRender = /* @__PURE__ */ __name((_elem, graph, diagramType, id, parentCluster, siteConfig) => __async(null, null, function* () {
  ...
  yield Promise.all(graph.nodes().map(function(v) {
    return __async(this, null, function* () {
      ...
      if (node?.clusterNode) {
        ...
        const o = yield recursiveRender(nodes, node.graph, diagramType, id, graph.node(v), siteConfig);
        const newEl = o.elem;
        ...
        if (graph.children(v).length > 0) {
          ...
          yield insertNode(nodes, graph.node(v), {
            config: siteConfig,
            dir
          });
        }
  ...
  const processEdges = /* @__PURE__ */ __name(() => __async(null, null, function* () {
    const edgePromises = graph.edges().map(function(e) {
      return __async(this, null, function* () {
        const edge = graph.edge(e.v, e.w, e.name);
        log.info("Edge " + e.v + " -> " + e.w + ": " + JSON.stringify(e));
        log.info("Edge " + e.v + " -> " + e.w + ": ", e, " ", JSON.stringify(graph.edge(e)));
        log.info("Fix", clusterDb, "ids:", e.v, e.w, "Translating: ", clusterDb.get(e.v), clusterDb.get(e.w));
        yield insertEdgeLabel(edgeLabels, edge);
      });
    });
  ...
  yield Promise.all(sortNodesByHierarchy(graph).map(function(v) {
    return __async(this, null, function* () {
      const node = graph.node(v);
      ...
      if (node?.clusterNode) {
        ...
        if (graph.children(v).length > 0) {
          ...
          yield insertCluster(clusters, node);
          clusterDb.get(node.id).node = node;
        } else {
...
var render = /* @__PURE__ */ __name((data4Layout, svg) => __async(null, null, function* () {
  ...
  yield recursiveRender(element, graph, data4Layout.type, data4Layout.diagramId, void 0, siteConfig);
}), "render");
export {
  render
};
//# sourceMappingURL=chunk-VG2L7XFG.js.map
