...
var Xe = __spreadProps(__spreadValues({}, X), {
...
var Je = __spreadProps(__spreadValues({}, N), {
    ...
    for (; e; ) {
      ...
      if (this.options.extensions?.startInline) {
        ...
        this.options.extensions.startInline.forEach((h) => {
          g = h.call({
            lexer: this
          }, c), typeof g == "number" && g >= 0 && (u = Math.min(u, g));
  ...
  parse(e, t = true) {
    ...
    for (let r = 0; r < e.length; r++) {
      ...
      if (this.options.extensions?.renderers?.[i.type]) {
        let a = i, o = this.options.extensions.renderers[a.type].call({
          parser: this
        }, a);
        ...
        for (let s in n.hooks) {
          ...
          S.passThroughHooks.has(s) ? i[a] = (u) => {
            ...
            if (this.defaults.async) return (() => __async(this, null, function* () {
              let g = yield o.apply(i, u);
              return g === false && (g = yield p.apply(i, u)), g;
            }))();
            let c = o.apply(i, u);
            return c === false && (c = p.apply(i, u)), c;
          };
  ...
  parseMarkdown(e) {
    return (n, r) => {
      ...
      if (s.hooks && (s.hooks.options = s, s.hooks.block = e), s.async) return (() => __async(this, null, function* () {
        let o = s.hooks ? yield s.hooks.preprocess(n) : n, u = yield (s.hooks ? yield s.hooks.provideLexer() : e ? x.lex : x.lexInline)(o, s), c = s.hooks ? yield s.hooks.processAllTokens(u) : u;
...
var getRegisteredIconData = /* @__PURE__ */ __name((iconName, fallbackPrefix) => __async(null, null, function* () {
  ...
  if (!icons) {
    ...
    try {
      ...
      icons = __spreadProps(__spreadValues({}, loaded), {
        prefix
      });
...
var isIconAvailable = /* @__PURE__ */ __name((iconName) => __async(null, null, function* () {
  try {
    yield getRegisteredIconData(iconName);
    return true;
  } catch {
    return false;
  }
}), "isIconAvailable");
var getIconSVG = /* @__PURE__ */ __name((iconName, customisations, extraAttributes) => __async(null, null, function* () {
  let iconData;
  try {
    iconData = yield getRegisteredIconData(iconName, customisations?.fallbackPrefix);
  } catch (e) {
    log.error(e);
...
function addHtmlSpan(_0, _1, _2, _3) {
  return __async(this, arguments, function* (element, node, width, classes, addBackground = false, config = getConfig()) {
    ...
    const sanitizedLabel = hasKatex(node.label) ? yield renderKatexSanitized(node.label.replace(common_default.lineBreakRegex, "\n"), config) : sanitizeText(node.label, config);
...
function replaceIconSubstring(_0) {
  return __async(this, arguments, function* (text, config = {}) {
    const pendingReplacements = [];
    text.replace(/(fa[bklrs]?):fa-([\w-]+)/g, (fullMatch, prefix, iconName) => {
      pendingReplacements.push((() => __async(null, null, function* () {
        const registeredIconName = `${prefix}:${iconName}`;
        if (yield isIconAvailable(registeredIconName)) {
          return yield getIconSVG(registeredIconName, void 0, {
            class: "label-icon"
          });
        } else {
    ...
    const replacements = yield Promise.all(pendingReplacements);
    return text.replace(/(fa[bklrs]?):fa-([\w-]+)/g, () => replacements.shift() ?? "");
  });
}
__name(replaceIconSubstring, "replaceIconSubstring");
var createText = /* @__PURE__ */ __name((_0, ..._1) => __async(null, [_0, ..._1], function* (el, text = "", {
  style = "",
  isTitle = false,
  classes = "",
  useHtmlLabels = true,
  ...
  if (useHtmlLabels) {
    ...
    const decodedReplacedText = yield replaceIconSubstring(decodeEntities(htmlText), config);
    ...
    const vertexNode = yield addHtmlSpan(el, node, width, classes, addSvgBackground, config);
    return vertexNode;
  } else {
...
//# sourceMappingURL=chunk-XXV2XYBW.mjs.map
