...
function executeCoseBilkentLayout(data, _config) {
  return __async(this, null, function* () {
    log.debug("Starting cose-bilkent layout algorithm");
    try {
      validateLayoutData(data);
      const cy = yield createCytoscapeInstance(data);
...
var render = /* @__PURE__ */ __name((_0, _1, _2, _3) => __async(null, [_0, _1, _2, _3], function* (data4Layout, svg, {
  insertCluster,
  insertEdge,
  insertEdgeLabel,
  insertMarkers,
  ...
  yield Promise.all(data4Layout.nodes.map((node) => __async(null, null, function* () {
    if (node.isGroup) {
      const clusterNode = __spreadValues({}, node);
      clusterDb[node.id] = clusterNode;
      nodeDb[node.id] = clusterNode;
      yield insertCluster(subGraphsEl, node);
    } else {
      const nodeWithPosition = __spreadValues({}, node);
      nodeDb[node.id] = nodeWithPosition;
      const nodeEl = yield insertNode(nodes, node, {
  ...
  const layoutResult = yield executeCoseBilkentLayout(updatedLayoutData, data4Layout.config);
  ...
  yield Promise.all(data4Layout.edges.map((edge) => __async(null, null, function* () {
    const _edgeLabel = yield insertEdgeLabel(edgeLabels, edge);
...
//# sourceMappingURL=chunk-EWAUG72J.mjs.map
