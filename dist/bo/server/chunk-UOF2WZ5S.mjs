...
var populateDb = /* @__PURE__ */ __name((ast, db) => {
  ...
  ast.links.forEach((link) => {
    ...
    const {
      flow: arrowFlow,
      label: flowLabel
    } = extractFlowFromArrow(link.arrow);
    if (!flow && arrowFlow) {
      flow = arrowFlow;
    }
...
var parser = {
  ...
  parse: /* @__PURE__ */ __name((input) => __async(null, null, function* () {
    const ast = yield parse("wardley", input);
    log.debug(ast);
...
var WardleyBuilder = class {
  ...
  addNode(node) {
    ...
    const merged = __spreadProps(__spreadValues(__spreadValues({}, existing), node), {
  ...
  setAxes(partial) {
    this.axes = __spreadValues(__spreadValues({}, this.axes), partial);
  }
...
//# sourceMappingURL=chunk-UOF2WZ5S.mjs.map
